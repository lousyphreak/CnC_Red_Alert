...
BOOL next_message(MSG* message, bool remove, bool wait)
{
    for (;;) {
        ...
        switch (event.type) {
            case SDL_EVENT_QUIT:
                translated.message = WM_CLOSE;
                translated.hwnd = first_window();
                compat_trace("SDL_EVENT_QUIT -> WM_CLOSE hwnd=%p", translated.hwnd);
                if (!translated.hwnd) {
                    translated.message = WM_QUIT;
                    compat_trace("SDL_EVENT_QUIT -> WM_QUIT (no window)");
                }
                break;
            case SDL_EVENT_WINDOW_CLOSE_REQUESTED:
                translated.message = WM_CLOSE;
                translated.hwnd = window_for_id(event.window.windowID);
                compat_trace("SDL_EVENT_WINDOW_CLOSE_REQUESTED -> WM_CLOSE hwnd=%p windowID=%u", translated.hwnd,
                    static_cast<unsigned>(event.window.windowID));
                break;
            case SDL_EVENT_WINDOW_FOCUS_GAINED:
                translated.message = WM_ACTIVATEAPP;
                translated.hwnd = window_for_id(event.window.windowID);
                translated.wParam = TRUE;
                compat_trace("SDL_EVENT_WINDOW_FOCUS_GAINED -> WM_ACTIVATEAPP hwnd=%p", translated.hwnd);
                break;
            case SDL_EVENT_WINDOW_FOCUS_LOST:
                translated.message = WM_ACTIVATEAPP;
                translated.hwnd = window_for_id(event.window.windowID);
                translated.wParam = FALSE;
...
BOOL ClipCursor(const RECT* rect)
{
    ...
    if (!rect) {
        return SDL_SetWindowMouseRect(window->sdl_window, nullptr) ? TRUE : FALSE;
    }

...
- Audit follow-up on the staged CD-removal / local-install branch fixed one remaining secondary-root edge case:
  ...
  - practical effect: the staged local-search-root fallback still works for installs with loose expansion packets/maps beside the executable, while direct-`GameData/` launches no longer inject a bogus filesystem error state into the same menu flow.
...
- Removed the remaining CD-era asset/runtime layer so the SDL3 port now assumes a fixed local `GameData/` install and keeps DVD-specific behavior limited to movies behind a compile-time gate:
  ...
  - follow-up fix for the first pass: the real local install layout still uses split top-level base archives (`MAIN1.MIX`..`MAIN4.MIX`) rather than a merged top-level `MAIN.MIX`; `CODE/STARTUP.CPP` now resolves the asset root to the actual `GameData/` directory relative to the executable (supporting both binaries launched from inside `GameData/` and from sibling build directories), and `CODE/INIT.CPP` now registers those fixed local `MAIN1`..`MAIN4` archives before validating logical inner mixes like `CONQUER.MIX`, `GENERAL.MIX`, `SOUNDS.MIX`, and the movie archives;
  - validation for this checkpoint: `cmake --build build --target redalert -- -j2` succeeds after the full CD/runtime removal pass.

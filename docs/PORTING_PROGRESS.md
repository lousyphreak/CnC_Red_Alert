...
- Started the Westwood Online replacement networking track (tracked in `docs/WOL_PROTOCOL.md` and in `server/`):
  ...
  - new `CODE/WSCLIENT.{H,CPP}` WebSocket client has two back-ends behind a single interface: **native** uses the existing `CODE/SOCKETS.H` seam with a local RFC 6455 implementation (inline SHA-1 + base64 for the handshake, XOR masking on client frames, ping/pong/close handled internally), and **emscripten** uses `emscripten/websocket.h` (link with `-sUSE_WEBSOCKET=1`);
  ...
  - new CMake option `RA_ENABLE_WOL` (default ON, propagated as `RA_ENABLE_WOL=1`) and the emscripten link flag `-sUSE_WEBSOCKET=1` are wired in `CMakeLists.txt`;
...
- WOL GUI integration pass (this checkpoint):
  ...
  - `SessionClass` (`CODE/SESSION.{H,CPP}`) gained `char WolServer[256]`, persisted under `[MultiPlayer] WolServer=` via `Read_/Write_MultiPlayer_Settings`. On emscripten the field auto-derives from `window.location` via `EM_ASM_INT` (picks `ws://` or `wss://` to match the page scheme, falls back to `/ws` on the same host) the first time the setting is loaded empty, so the hosted build never needs a manual entry.

...
- Reworked the Emscripten asset-delivery path so the browser build no longer depends on a monolithic multi-gigabyte preload bundle:
  ...
  - important remaining browser follow-up: the initial page download is now much smaller and large `.MIX` archives no longer have to be fetched whole on first remote open, but this checkpoint still needs an actual in-browser runtime smoke test to measure the real startup range-request pattern and to decide whether sparse `.MIX` chunk persistence across reloads is worth adding on top of the current in-memory shared cache.

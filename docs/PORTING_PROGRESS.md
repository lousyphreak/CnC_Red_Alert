...
- Removed the remaining Win32 window-management compatibility wrappers and switched the live main-window path to explicit SDL handling:
  ...
  - replaced `Create_Main_Window(...)` with a direct SDL-backed `RA_CreateWindow(...)` call that installs `Windows_Procedure` as the message target while letting `WINSTUB` drive show/maximize/raise behavior through SDL itself instead of a Win32 wrapper layer;
  ...
  - deleted the actual compat declarations/definitions for `WNDCLASS`, `RegisterClass`, `CreateWindowEx`, `ShowWindow`, `SetForegroundWindow`, `UpdateWindow`, `SetFocus`, and `DefWindowProc` from `SDL3_COMPAT/wrappers/win32_compat.{h,cpp}`; the remaining compat message queue (`PeekMessage` / `GetMessage` / `SendMessage` / `PostMessage`) is still intentional for non-window async notifications such as the legacy socket/event path;

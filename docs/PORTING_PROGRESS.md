...
- Fixed the browser WOL immediate post-start desync regression (2026-04-21):
  ...
  - Fix implemented:
    - `CODE/NETDLG.CPP` now tracks the authoritative Zig `game_id` alongside each WOL lobby game entry;
    - `Request_To_Join(...)` now sends `GAME_JOIN` before the legacy join request so the server-side game membership is in place before `NET_GO` / gameplay startup;
    - the join dialogs now send `GAME_LEAVE` again on WOL cancel/reject/unjoin paths and clear their tracked game-id table on entry/exit so stale room membership does not leak across attempts.
  - Result: browser guests are part of the Zig game room before the host flips the room to started state, so started-game relay reaches both peers instead of timing out on "Waiting for connections...".
  - validation for this checkpoint: native `cmake --build build --target redalert -j2`, Emscripten `cmake --build build-emscripten --target redalert -j2`, and `tests/run_wol_transport_tests.sh` all pass after the WOL game-membership fix.

- Fixed the deeper browser WOL lobby roster/game-list expiry bug (2026-04-21):
  - `CODE/NETDLG.CPP` was already consuming Zig `CHANNEL_*` and `GAME_LIST_REPLY` control frames, but the normal join dialog still fed those WOL-derived entries through the original UDP-era timeout cleanup.
  - Root cause: the classic join dialog deletes `Session.Chat` entries after ~360 ticks and `Session.Games` entries after ~400 ticks unless their `LastTime` fields keep being refreshed by periodic broadcast traffic. That model fits `NET_CHAT_ANNOUNCE` / `NET_ANSWER_GAME`, but Zig channel membership and game-list replies are event-driven rather than heartbeat-driven, so peers and games could appear and then silently vanish again even while `/ws` traffic stayed healthy.
  - Fix implemented:
    - skip the old time-based chat/game expiry path for `GAME_WOL` in the normal join dialog;
    - reconcile `GAME_LIST_REPLY` as an authoritative snapshot, removing games that are no longer present on the server instead of relying on local timeout aging.
  - Result: separate browsers now keep seeing each other in the lobby and keep seeing open games until the Zig server sends an explicit membership or game-list change, which matches the actual WOL control-plane contract.
  - validation for this checkpoint: native `cmake --build build --target redalert -j2`, Emscripten `cmake --build build-emscripten --target redalert -j2`, and `tests/run_wol_transport_tests.sh` all pass after the authoritative-lobby-state fix.

- Fixed Zig WOL server 100% CPU spin after early client disconnects (2026-04-21):
  - `server/src/server.zig` now tracks peer EOF/write-failure state explicitly on each connection and drops pending transmit bytes as soon as the peer is known dead, even if that connection had already entered the normal `wants_close_after_flush` path for an HTTP/WebSocket response.
  - Root cause: Linux can keep reporting a dead socket as `POLLIN|POLLOUT` with `read(...) == 0` instead of raising `POLLHUP`. For requests like `GET /healthz` with `Connection: close`, the server queued a response, marked the connection for flush-and-close, then observed EOF from the client. Because the old loop only treated `POLLHUP|POLLERR|POLLNVAL` as a hard disconnect, that dead socket stayed in the poll set forever with pending `tx`, producing an immediate ~100% CPU busy loop.
  - Added focused Zig regression tests around the close bookkeeping so live graceful-close paths still retain pending `tx`, while EOF/fatal-close paths now discard unsendable bytes and let the connection be removed promptly.
  - validation for this checkpoint: `cd server && zig build && zig build test && zig build test-integration` all pass, `tests/run_wol_transport_tests.sh` still passes, and the local reproducer that previously burned ~99 scheduler ticks in 1 second after an early-disconnect `/healthz` request now stays idle (`delta_ticks_1s=0`).

- Fixed the intermittent browser WOL lobby/game visibility race (2026-04-21):
  - `CODE/SESSION.CPP::Compute_Unique_ID()` now mixes per-load browser entropy into the legacy machine-id heuristic on Emscripten. Two browser tabs launched on the same machine could otherwise collide on `Session.UniqueID`, which made `NET_CHAT_ANNOUNCE` packets from the other tab look like "my own" announce and intermittently hid peers from the lobby chat roster.
  - `CODE/NETDLG.CPP` now consumes the Zig server's authoritative WOL control frames while the classic join dialog is open: `CHANNEL_JOINED` / `CHANNEL_USER_JOIN` / `CHANNEL_USER_LEAVE` keep `Session.Chat` in sync with the actual lobby membership, and `GAME_LIST_REPLY` keeps `Session.Games` refreshed from the server-side room list.
  - `CODE/NETDLG.CPP::Net_New_Dialog()` now registers host-created WOL games with the Zig server via `GAME_CREATE`, marks them started via `GAME_START` when launching, and leaves the server-side room on cancel. That makes newly opened browser games visible through the same control-plane that already knows the real channel membership instead of waiting purely on periodic legacy `NET_QUERY_GAME` / `NET_ANSWER_GAME` timing.
  - The actual join/start/gameplay packets stay on the legacy lobby/gameplay flow, so this is a deterministic discovery fix rather than a gameplay refactor: the old UI still joins via `NET_QUERY_PLAYER` / `NET_QUERY_JOIN`, but the roster/game list no longer depends on lossy timing assumptions during browser startup.
  - validation for this checkpoint: native `cmake --build build --target redalert -j2`, Emscripten `cmake --build build-emscripten --target redalert -j2`, and `tests/run_wol_transport_tests.sh` all pass after the browser-identity and lobby-control updates.

- Fixed the remaining browser WOL lockstep/disconnect regression in the gameplay receive path (2026-04-21):
  - `CODE/QUEUE.CPP::Wait_For_Players()` now resets `packetlen` to `Session.MetaSize` before **every** `Get_Private_Message(...)` poll. The old loop passed an uninitialized length on the first iteration and then reused the previous packet's length on later iterations.
  - `CODE/{NETDLG,CONQUER}.CPP` now reset `Session.GPacketlen` to `sizeof(Session.GPacket)` before each `Get_Global_Message(...)` poll, and `CODE/SESSION.CPP` seeds that field in the session constructor.
  - Root cause: the legacy UDP path effectively ignored the caller-provided receive length, so these stale/uninitialized lengths went unnoticed for years. `WSManagerClass` correctly treats `*buflen` as the destination capacity; during browser/WOL startup that meant valid FRAMESYNC/gameplay packets could be dropped before `Process_Receive_Packet()` ever saw them, leaving the game stuck on "Waiting for connections..." even while WebSocket traffic was still flowing.
  - `CODE/QUEUE.CPP::Process_Receive_Packet()` now also ignores packets from unknown connection ids instead of indexing `their_frame[]` / `their_sent[]` / `their_recv[]` with an invalid connection slot.
  - `CODE/QUEUE.CPP` now treats `GAME_WOL` like `GAME_INTERNET` for the frame-sync timeout factor so browser/WebSocket relay startup is not forced through the much tighter LAN timeout budget.
  - validation for this checkpoint: `tests/run_wol_transport_tests.sh` still passes, and both native `build/` plus configured Emscripten `build-emscripten/` `redalert` targets rebuild cleanly after the receive-path and timeout changes.

- Fixed browser WOL false-disconnect during match startup (2026-04-21):
  - `CODE/WSMGR.{H,CPP}` no longer drops inbound private relay traffic just because the sender's WOL cid has not been mapped to a gameplay connection id yet. Private packets that arrive in that window are now queued by source cid and moved into the normal per-connection receive queue as soon as `Create_Connection_By_Cid(...)` installs the mapping.
  - Root cause: browser peers can reach the started-game relay path and begin sending frame-sync/gameplay packets while the slower side is still finishing scenario startup and has not yet run `Session.Create_Connections()` / `Wol.Create_Connection(...)`. The old code treated that as "unknown sender" and silently discarded the packet even though the WebSocket session was healthy, which made lockstep timeout logic conclude the peer had stopped responding.
  - Pending private packets are also purged on peer leave/reset so early queued traffic cannot survive disconnect teardown under a later reused connection slot.
  - `tests/wsmgr_e2e.cpp` now covers the exact race by delivering a gameplay packet before the receiver creates its connection mapping and verifying the packet becomes visible only after the mapping is installed.
  - validation for this checkpoint: `tests/run_wol_transport_tests.sh` passes with the new deferred-packet coverage, native `cmake --build build --target redalert -j2` still builds cleanly, and the configured Emscripten build at `build-emscripten/` still rebuilds `redalert` successfully.

- Fixed browser WOL/gameplay frame starvation in active network matches (2026-04-21):
  - `CODE/QUEUE.CPP::Wait_For_Players()` now yields through `SDL_GameInput_Delay(1)` on Emscripten at the end of each lockstep wait iteration.
  - Root cause: once a browser network game reached the in-game frame-sync wait, the old synchronous `while (1)` loop never returned control to the browser event loop. That meant the same code that was waiting for WebSocket gameplay packets and fresh presents also prevented the browser from dispatching those callbacks, producing a hot 100% CPU spin and visible multi-second frame stalls.
  - The fix stays browser-only and reuses the existing SDL input/yield seam instead of introducing another direct Emscripten special case; native multiplayer timing remains unchanged.

- Emscripten now defaults multiplayer to the WOL/WebSocket backend (2026-04-21):
  - `CMakeLists.txt` now forces `RA_ENABLE_WOL=ON` for Emscripten builds alongside the existing `RA_ENABLE_UDP=OFF` / `RA_ENABLE_TCP=OFF`, so browser builds always compile the new WOL-style transport by default.
  - `CODE/MPLAYER.CPP::Select_MPlayer_Game()` now defaults the highlighted multiplayer choice to `BUTTON_WOL` on Emscripten when WOL is the only live network backend, so the browser build's default multiplayer action lands on the Zig-hosted WebSocket flow instead of skirmish.
  - validation for this checkpoint: native and Emscripten `redalert` builds still configure/build cleanly after the default-selection change, and the Emscripten configuration path now reports UDP/TCP disabled with WOL forced on.

- Emscripten WOL now always follows the current page host (2026-04-20):
  - `CODE/SESSION.CPP` no longer only seeds `Session.WolServer` from `window.location` when the saved browser config entry is empty. On Emscripten it now re-derives the URL on every settings load as `ws://<current host>/ws` or `wss://<current host>/ws`, so a deployment hosted on `ra.example.com` always connects back to `ra.example.com` for WOL traffic.
  - This fixes a browser-persistence mismatch: `WolServer` is stored in the config/IDB-backed settings, so the earlier "only if empty" logic could preserve a stale previous host across restarts or after moving the hosted build to a new hostname.
  - validation for this checkpoint: both native and Emscripten `redalert` targets build cleanly after the same-host override change, and the generated Emscripten output still contains the `window.location`-based WOL endpoint derivation used by the browser runtime.

- Docker/browser hosting now runs through the Zig WOL server as the single public HTTP endpoint (2026-04-20):
  - `server/src/{http,server,main,integration_test}.zig` now covers the old browser-container contract directly: `/healthz` is served without auth, `/` redirects to `/redalert.html`, `/GameData/...` lookups work with the original uppercase path (and lowercase compatibility), optional image-level basic auth now protects both static hosting and the `/ws` upgrade through `RA_BASIC_AUTH_USERNAME` / `RA_BASIC_AUTH_PASSWORD`, and static serving now implements single-range `206 Partial Content` / `Content-Range` / `Accept-Ranges` responses so the browser WWFS lazy-fetch path can stream `GameData` correctly from the combined server.
  - `Dockerfile` now builds `server/` with Zig in its own stage and the final `web-runtime*` images run `/usr/local/bin/ra-wol-server` directly instead of nginx, serving the Emscripten output and (for `web-runtime-with-gamedata`) the filtered `GameData` tree from the same port `80` that also carries `/ws`.
  - `README.md`, `docs/WOL_PROTOCOL.md`, and `k8s/ea-games/redalert-web*.yaml` were updated to match the new one-port Zig-hosted deployment; the sample Kubernetes secret now stores raw username/password fields because the nginx-only `RA_BASIC_AUTH_HTPASSWD` preprocessing path is gone with the server swap.
  - validation for this checkpoint: `cd server && zig build test && zig build test-integration` both pass, including new integration coverage for root redirect, health probe, uppercase/lowercase `GameData` paths, HTTP range serving, static basic auth, and authenticated WebSocket upgrade.

- WOL transport audit / hardening follow-up (2026-04-20):
  - Audited the staged WOL/WebSocket client/server changes end-to-end against the existing ASAN build, the live transport runner (`tests/run_wol_transport_tests.sh`), and the Zig server tests. The audit turned up several non-obvious correctness issues that the original happy-path tests did not cover.
  - `CODE/WSMGR.CPP` now purges already-queued private relay messages when a peer disconnects, so gameplay packets from a departed peer cannot survive behind `OP_PEER_LEFT` and get delivered later under a dead connection id.
  - `CODE/WSMGR.CPP` also now hardens inbound/control buffering: short `OP_WELCOME` payloads are rejected before decoding, queued private/global/control payloads are capped to the manager's message limit, `Get_Private_Message` / `Get_Global_Message` now honor the caller-provided buffer capacity instead of blindly memcpy'ing into `Session.MetaPacket`-sized buffers, and `Next_Control_Frame` now reports the number of bytes actually copied instead of the original queued length after truncation.
  - `CODE/WSCLIENT.CPP` native reconnects now reset their internal handshake/buffer state fully on close/reconnect; without that, a logout/cancel followed by another connect could leave `handshake_sent` latched and stall forever waiting for a `WELCOME` that could never arrive. `tests/wsmgr_e2e.cpp` now covers the send-then-disconnect stale-packet case and the subsequent reconnect path.
  - `server/src/server.zig` now rejects unexpected WebSocket text frames at the WS layer instead of trying to parse them as WOL app frames, and `handleRelayBroadcast` now stops relaying started-game traffic to channel-only lobby members. Pre-start lobby discovery still uses the channel/game union; in-game relay is now game-members-only.
  - `tests/run_wol_transport_tests.sh` was hardened for repeatable local runs: the standalone test binaries now link with `-pthread`, server logs go to a unique `mktemp` path, cleanup removes the temp log, and the script waits for actual server readiness instead of assuming a fixed startup sleep is enough.

- WOL restart / teardown fixes (2026-04-20):
  - `CODE/CONNMGR.H` now exposes a virtual `Shutdown()` hook so stateful transports can tear themselves down explicitly when a match or lobby session ends.
  - `CODE/WSMGR.{H,CPP}` now has real lifecycle management instead of assuming a single process-long session: `Shutdown()` sends `OP_GAME_LEAVE`, flushes/polls briefly so the frame reaches the server, closes the underlying `WSClientClass`, and clears the connection roster, queued private/global/control messages, `LoggedIn`, `Connected`, and `MyClientId`. `Init()` now detects the "already logged in to the same WOL server/nick" case and reuses that session rather than always reconnecting, while still forcing a clean reconnect when the server URL or nickname changed.
  - `CODE/CONQUER.CPP` no longer fully disconnects WOL when a mission ends normally. Instead it clears `Session.GameName` and calls `Wol.Return_To_Lobby()`, which sends `GAME_LEAVE`, drains any resulting control traffic, clears the per-game peer/queue state, but **keeps** the logged-in lobby session alive. This matches the old WOL behavior more closely: after game end, return to the multiplayer/lobby flow instead of forcing a fresh login.
  - `CODE/NETDLG.CPP::Shutdown_Network()` now calls `Pick_NetMgr(); NetMgr->Shutdown();` after clearing `Session.GameName`, so leaving a WOL game actually tears down the transport instead of only blanking the advertised game name.
  - `CODE/INIT.CPP` `GAME_WOL` cancel/failure path now calls `Shutdown_Network()` before returning to the main menu, closing the WOL session if the user backs out of the lobby instead of leaving a stale socket alive for the next attempt.
  - `server/src/server.zig` `removeFromGame()` no longer dereferences a destroyed `Game` after the last member leaves; the new client `GAME_LEAVE` path exposed this latent use-after-destroy / invalid-state bug immediately in the transport tests.
  - `server/src/integration_test.zig` now exercises `GAME_LEAVE` and verifies the host sees `PEER_LEFT`, an updated one-member `GAME_MEMBERS`, and the subsequent `GAME_LIST_REPLY`, so the restart/leave path is covered by the real end-to-end server test. `tests/wsmgr_e2e.cpp` now also exercises the client-side "return to lobby, then start a second game on the same live WOL session" flow.

- In-game gameplay flow for `GAME_WOL` (2026-04-19):
  - **Critical root cause**: `CODE/QUEUE.CPP` `Queue_AI()` dispatched the lockstep AI loop via a `switch (Session.Type)` that only listed `GAME_SKIRMISH`/`GAME_NORMAL`/`GAME_UDP`/`GAME_INTERNET` — `GAME_WOL` fell through, so `Queue_AI_Multiplayer()` was never called. Without that, the multiplayer lockstep never runs: no local player events get queued, no network packets are read, the menu key, mouse clicks, unit commands etc. all produce no effect. Added `case GAME_WOL:` alongside the existing UDP/INTERNET cases so the multi-player tick runs for WOL sessions.
  - Added `GAME_WOL` to every other gameplay-time `Session.Type == GAME_UDP || Session.Type == GAME_INTERNET` site that gates network-loop behavior: `QUEUE.CPP` (`Generate_Timing_Event` FrameInfo delay computation, `Handle_Timeout`'s oldest-connection destroy, the inner `Destroy_Connection` call, the `EventClass::EXIT` connection teardown, the out-of-sync connection shutdown) and `CONQUER.CPP` (`Main_Game` `Shutdown_Network` on exit, F1–F8 "to all"/"to player" chat addressing, chat-send path, `UDP_Call_Back` invocation in `Call_Back`), plus `HOUSE.CPP`'s winner-cleanup connection drain. All use `#if RA_ENABLE_WOL / || Session.Type == GAME_WOL / #endif` to keep the non-WOL build unchanged.
  - `GAME_INTERNET`-only sites (statistics, ladder, `pWolapi`, tournament/disconnect-pinging, `PlayingAgainstVersion` non-Aftermath map cap) were intentionally left alone because they're Westwood-Online-specific legacy paths that don't apply to the new Zig relay transport.

- WOL lobby discovery fixes (2026-04-18):
  - `CODE/WSMGR.CPP` `Configure()` now auto-appends `/ws` to the server URL if the caller passes a bare `ws://host:port`, so `-wol-server ws://127.0.0.1:8070` routes to the upgrade endpoint instead of returning 404. `Init()` now logs `[WOL]` connect/handshake/WELCOME progress and errors to stderr and exits early on `STATE_FAILED`/`STATE_DISCONNECTED` rather than waiting the full 6s timeout. After `WELCOME` it sends a `CHANNEL_JOIN "ra-lobby"` and drains events for 500ms so subsequent broadcasts have channel scope on the server.
  - `CODE/WSMGR.CPP` `Get_Global_Message` now fills `product_id=0xaa00` (`UDPGlobalConnClass::COMMAND_AND_CONQUER0`) so `NETDLG.CPP`'s `Session.GProductID` filter accepts WOL global packets; previously every lobby broadcast was silently dropped.
  - `CODE/SESSION.CPP` `Create_Connections()` now handles `GAME_WOL` (was UDP/INTERNET only, hardcoded to `Udp.Create_Connection`) and branches to `Wol.Create_Connection`. Added free function `Active_Net_Mgr()` (declared in `CODE/SESSION.H`) that returns `&Wol` for `GAME_WOL` and `&Udp` otherwise.
  - Sed-replaced all direct `Udp.<method>` references in `CODE/CONQUER.CPP`, `CODE/HOUSE.CPP`, `CODE/SENDFILE.CPP` to `Active_Net_Mgr()-><method>` so gameplay, scenario transfer and lobby polling outside `NETDLG` route to the active transport. `NETDLG.CPP` still uses its file-scope `static ConnManClass *NetMgr` / `Pick_NetMgr()` pattern because it's set once per dialog entry. `Set_Bridge` is explicitly left as `Udp.Set_Bridge` (no WOL equivalent).
  - Fixed `#ifdef RA_ENABLE_WOL` → `#if RA_ENABLE_WOL` in `CODE/QUEUE.CPP` so the in-game WOL net-pointer selection is actually active when the feature is enabled.
  - `server/src/server.zig` `handleRelayBroadcast` now broadcasts to the **union** of game members and channel members (deduplicated, self excluded). Previously broadcasts only went to game members if the caller was in a game, which broke lobby discovery once the host created a game: joiners still in the lobby channel never saw the host's `NET_ANSWER_GAME` replies to their `NET_QUERY_GAME`. `handleRelayPrivate` accepts targets that share a game OR a channel.
  - `server/src/integration_test.zig` updated to match current `handleGameJoin` ordering (`game_list_reply` delivered before `game_members` now that lobby broadcasts reach channel members).

- Started the Westwood Online replacement networking track (tracked in `docs/WOL_PROTOCOL.md` and in `server/`):
  - new Zig 0.15.x server under `server/` acts as a one-binary combined HTTP static host (serves the emscripten build + `GameData/` with case-insensitive lookup) and WebSocket control-plane/relay endpoint that implements a WOL-shaped protocol (login, channel list, channel join, chat, game rooms, and a full-gameplay byte relay);
  - wire protocol is length-prefixed binary frames `u16 opcode | u16 flags | u32 payload_len | payload[]` inside WebSocket binary messages, with `HEADER_SIZE = 8`, `PROTOCOL_VERSION = 1`, `MAX_PAYLOAD = 1 MiB`; canonical opcode table lives in `server/src/proto.zig` and `CODE/WOL_PROTO.H`;
  - the server uses stdlib only (no third-party deps); framing, WebSocket (RFC 6455) and HTTP parsing each have unit tests, and `zig build test-integration` spawns the real binary and drives two clients end-to-end through `HELLO`, `GAME_CREATE`, `GAME_JOIN`, `RELAY_BROADCAST`, and `RELAY_PRIVATE`;
  - shared C++ protocol header `CODE/WOL_PROTO.H` mirrors the Zig opcode table and provides inline little-endian read/write helpers;
  - new `CODE/WSCLIENT.{H,CPP}` WebSocket client has two back-ends behind a single interface: **native** uses the existing `CODE/SOCKETS.H` seam with a local RFC 6455 implementation (inline SHA-1 + base64 for the handshake, XOR masking on client frames, ping/pong/close handled internally), and **emscripten** uses `emscripten/websocket.h` (link with `-lwebsocket.js` on current Emscripten releases);
  - new `CODE/WSMGR.{H,CPP}` (`WSManagerClass : public ConnManClass`, plus a new `Wol` global next to `Udp`) wraps one `WSClientClass` and bridges the game's abstract connection-manager contract onto the WebSocket relay — `Send_Private_Message` maps to `RELAY_PRIVATE` (or `RELAY_BROADCAST` for `CONNECTION_NONE`), `Get_Private_Message` drains a per-peer queue keyed on `RELAY_FROM`, control-plane frames (HELLO/WELCOME/GAME_*) land in a separate queue the CLI launch path can pop;
  - `CODE/SESSION.H` now has `GAME_WOL = 6`, and `CODE/QUEUE.CPP` routes `Session.Type == GAME_WOL` through `net = &Wol` (alongside the existing `GAME_UDP`/`GAME_INTERNET` branch), so the in-game lockstep loop can drive the WebSocket transport without any further changes;
  - server-side ordering fix: `handleGameJoin` now broadcasts the updated game list **before** sending `GAME_MEMBERS` to members, matching the order already used by `handleGameCreate` so clients can parse `GAME_LIST_REPLY → GAME_MEMBERS` without reordering;
  - new CMake option `RA_ENABLE_WOL` (default ON, propagated as `RA_ENABLE_WOL=1`) and the emscripten WebSocket support link (`-lwebsocket.js` on the current SDK) are wired in `CMakeLists.txt`;
  - validation for this checkpoint: `zig build` + `zig build test` + `zig build test-integration` all pass; native `cmake --build build --target redalert` builds cleanly with `WSMGR.CPP` and `WSCLIENT.CPP` picked up by the `CODE/*.CPP` glob; `tests/run_wol_transport_tests.sh` (which spawns the live server and runs `ws_smoke`, `wsmgr_e2e`, and `wsmgr_multi` at 4 and 6 clients) passes end-to-end and exercises the full multi-peer broadcast mesh through the relay.

- Still to do on the WOL track — the remaining work is game-UI integration, not transport. `QUEUE.CPP` already routes gameplay packets through `Wol` for `GAME_WOL`, but the pre-game setup (`NETDLG.CPP::Net_Fake_New_Dialog`/`Net_Fake_Join_Dialog`, `Init_Network`, `Server_Remote_Connect`/`Client_Remote_Connect`) is still hard-wired to UDP (~149 direct `Udp.` call sites), and replaces need to:
  1. parameterize the fake-dialog player/option negotiation on a generic `ConnManClass*` + add `Send_Global_Message`/`Get_Global_Message`/`Global_Response_Time` to the abstract base so `WSManagerClass` can plug in (Global messages map to `RELAY_BROADCAST`);
  2. add a CLI front-door for WOL (`-wol-server URL -wol-nick NAME -wol-host NAME [-wol-players N] | -wol-join NAME`) paralleling `Internet_Set_Command_Line_Host/Connect`, plus a `Wol_Has_Command_Line_Launch()` gate in `INIT.CPP`/`NETDLG.CPP`;
  3. populate `Session.Players`/`Session.NumPlayers`/`Session.ColorIdx`/`Session.Handle` from the `GAME_MEMBERS`/`GAME_STARTED` rosters and replace the UDP `NodeNameType::Address` usage in the fake dialogs with a transport-agnostic peer-id so WOL entries can populate it without a real UDP address;
  4. add the WOL lobby UI (`BUTTON_WOL` in `Select_MPlayer_Game`, minimal server-URL/nick/game-list/create-join screens) behind `RA_ENABLE_WOL` for the non-CLI path.
  The transport is ready for all of this — the remaining work is purely in the lobby/UI layer.

- WOL GUI integration pass (this checkpoint):
  - Promoted `ConnManClass` (`CODE/CONNMGR.H`) into a full abstract transport: `Init`, `Send_Global_Message`/`Get_Global_Message`/`Global_Num_Send`/`Global_Num_Receive`/`Global_Response_Time`/`Reset_Global_Response_Time`, `Create_Connection(int,char const*,UDPAddressClass*)`, `Delete_Connection`, `Connection_Address`/`Connection_Name`/`Connection_ID`/`Connection_Index`/`Num_Connections` are now virtual. `UDPManagerClass` (`CODE/UDPMGR.{H,CPP}`) overrides them as thin passthroughs and exposes the new `Reset_Global_Response_Time`.
  - `WSManagerClass` (`CODE/WSMGR.{H,CPP}`) implements the full `ConnManClass` surface. Channel disambiguation is client-side: a single tag byte (`0 = private gameplay`, `1 = global lobby`) is prepended to relay payloads so the Zig server stays a dumb byte-relay. WOL cids are packed little-endian into `UDPAddressClass::NodeAddress[0..3]` (NetworkNumber + NodeAddress[4..5] zeroed) so byte-wise equality and ordering work for all existing `UDPAddressClass` call sites. A `Configure(ws_url, nick)` entry-point stages the URL + handle, and `Init()` connects, sends `HELLO`, and spins on `Service()` until `WELCOME` arrives — no `SessionClass` coupling, so the standalone transport tests still build.
  - `SessionClass` (`CODE/SESSION.{H,CPP}`) gained `char WolServer[256]`, persisted under `[MultiPlayer] WolServer=` via `Read_/Write_MultiPlayer_Settings`. On emscripten the field auto-derives from `window.location` via `EM_ASM_INT` (picks `ws://` or `wss://` to match the page scheme, falls back to `/ws` on the same host); the later 2026-04-20 follow-up tightened this from initial empty-only seeding to a same-host rewrite on every browser load so persisted settings cannot drift stale across deployments.
  - Native CLI: `-wol-server URL` is parsed in `INIT.CPP`, stored into `Session.WolServer`, and written back via `Write_MultiPlayer_Settings` immediately so subsequent launches remember it.
  - Multiplayer menu: `CODE/MPLAYER.CPP::Select_MPlayer_Game` now exposes `BUTTON_WOL` under `RA_ENABLE_WOL` (independent of the dormant `WOLAPI_INTEGRATION` macro), returning `GAME_WOL`. `NUM_OF_BUTTONS` is a conditional expression so the layout grows only when the button is enabled. `INIT.CPP::SEL_MULTIPLAYER_GAME` handles `GAME_WOL` by calling `Init_Network()` + `Remote_Connect()` without allocating the UDP `PacketTransport`.
  - `NETDLG.CPP` was parameterized with a file-scoped `static ConnManClass *NetMgr` and a `Pick_NetMgr()` helper that picks `&Wol` when `Session.Type == GAME_WOL` and `&Udp` otherwise. All 149 existing `Udp.` method calls were rewritten to `NetMgr->` in one mechanical pass; the UDP-only `Set_Bridge` site was reverted to `Udp.Set_Bridge`. `Pick_NetMgr()` runs at the top of `Init_Network` and `Remote_Connect`, and the WOL branch in `Init_Network` now calls `Wol.Configure(Session.WolServer, Session.Handle)` before `Wol.Init()`.
  - Tests: `tests/run_wol_transport_tests.sh` keeps building `WSMGR.CPP` standalone (without dragging in `FUNCTION.H`) by linking a small `tests/udpaddr_stub.cpp` (the same byte-level `UDPAddressClass` ops without the game-global deps). `wsmgr_e2e`, `wsmgr_multi` (4 and 6 clients), and `ws_smoke` all pass against the live Zig server; `cmake --build build --target redalert` is clean.


- Completed a full Westwood Online archaeology/documentation pass and captured it in `docs/WOL_INVENTORY.md`:
  - the new document inventories the dormant WOL-specific files, the shared runtime/support files that still carry WOL behavior, and the UI helper files that were extended for the old WOL dialogs;
  - it reconstructs the original menu/login/chat/create-game/game-setup/download flow, explains the service/control-plane behavior separately from the actual gameplay network handoff, and includes ASCII UI mockups for the key screens;
  - the main architectural finding is that WOL in this tree was mostly a control-plane layer (login, channels, chat, game-room setup, paging, ladder/stats) wrapped around the normal `GAME_INTERNET` gameplay/session code, not a separate in-game transport stack;
  - the current tree still contains that front-end/glue code behind `WOLAPI_INTEGRATION`, but the build does not define the macro and the old `WolapiObject` bridge implementation is missing, so the feature is archival reference code rather than something that can be re-enabled directly.
...
- Finalized the browser `.MIX` sparse range-cache / look-ahead path so the shipped behavior is now coherent for both upgraded and completely clean browser stores:
  ...
  - `SDL3_COMPAT/wrappers/sdl_fs.cpp` now keeps blocking reads to the currently needed 512 KiB logical chunk, prefetches only the immediate next chunk in the background, keys the persistent sparse cache by the resolved asset URL, uses the current browser cache generation `redalert-wwfs-range-cache`, persists each logical chunk as chunk metadata plus 64 KiB subrecords, treats both `request.onerror` and thrown `request.result` reads as disposable cache misses, lets the blocking path consume resolved in-flight prefetch bytes directly, and makes the prefetch path check pending work plus metadata-only chunk presence before deciding to fetch;
  - practical browser effect: upgraded and clean stores both avoid stale-layout aliasing and browser large-value reopen failures, repeated reads through the current chunk no longer trigger warning storms for the same next chunk, and the next 512 KiB logical chunk is still downloaded ahead of need as its own background request instead of widening the current blocking read;
  - validation for this checkpoint: `cmake --build build --target redalert -- -j2`, `cmake --build build-asan --target redalert -- -j2`, `cmake --build build-emscripten --target redalert -- -j2`, `ctest --test-dir build --output-on-failure`, and `node --check build-emscripten/redalert.js` succeed after the final range-cache / prefetch cleanup.

- Fixed the Emscripten `.MIX` look-ahead path so browser prefetch no longer widens blocking chunk reads:
  - root cause: the previous read-ahead change still appended chunk `N+1` to the same synchronous range fetch that loaded chunk `N`, so each chunk miss could turn into a 1 MiB blocking wait and larger sequential reads could widen the same stall even further;
  - `SDL3_COMPAT/wrappers/sdl_fs.cpp` now keeps synchronous range loads to the single missing chunk the current read actually needs, then schedules a fire-and-forget prefetch for exactly the immediate next chunk after that chunk is accessed;
  - the browser range-cache bridge now also deduplicates in-flight next-chunk prefetches, so if the game reaches that next chunk before the background fetch finishes, the blocking path waits on the existing request instead of starting a duplicate download;
  - practical browser effect: touching chunk `N` no longer expands the current stall to include chunk `N+1`; the next chunk starts downloading early, but as its own background 512 KiB request instead of as part of the blocking read;
  - validation for this checkpoint: `cmake --build build --target redalert -- -j2`, `cmake --build build-asan --target redalert -- -j2`, `cmake --build build-emscripten --target redalert -- -j2`, and `ctest --test-dir build --output-on-failure` all succeed after the look-ahead scheduling fix.

- Tightened the web container packaging so browser deploys cache and ship only the runtime asset set they need:
  - `Dockerfile` now splits the nginx setup into `web-runtime-base`, keeps `web-runtime` as the plain HTML/JS/wasm image, and builds `web-runtime-with-gamedata` by copying the filtered `GameData/` tree **before** the Emscripten HTML/JS/wasm outputs so Docker can reuse the game-data layer when only the browser binary changes;
  - `docker/filter_emscripten_gamedata.py` still follows the `ra-assets-manifest.txt` case-preserving file list, but now also drops the top-level `GameData/RED ALERT/` Windows desktop-theme shell extras in addition to the existing browser-local/savegame/install/runtime-external exclusions, leaving only the actual game/runtime asset payload in the container;
  - validation for this checkpoint: the filtered docker asset copy now excludes the entire `GameData/RED ALERT/` subtree, and the `web-runtime-with-gamedata` image layout can place the reusable asset layer ahead of the HTML/JS/wasm copy steps.

- Smoothed the Emscripten range-backed archive stream for slower browser connections without changing archive semantics:
  - `SDL3_COMPAT/wrappers/sdl_fs.cpp` now uses 512 KiB logical `.MIX` range chunks for the browser stream, which is the chunk size the later sparse-cache / look-ahead follow-up work now builds on too;
  - validation for this checkpoint: `cmake --build build-emscripten --target redalert -j4` succeeds after the chunk-size/read-ahead adjustment.

- Finished the startup GameData-order cleanup so command-line selection now reaches the real bootstrap path instead of an obsolete setup gate:
  - `CODE/STARTUP.CPP` now honors only `-gamedata <dir>` as the first and highest-priority asset-directory choice, then falls back exactly to the process current working directory and finally the executable directory, stopping once one step exposes the required top-level `REDALERT.MIX` plus `MAIN1.MIX`..`MAIN4.MIX` archives;
  - all earlier sibling/parent `GameData/` heuristics and secondary launch-directory loose-file fallbacks remain removed, so the active search policy now matches the intended exact-order startup contract on native builds and on the Emscripten base-directory path;
  - the stale `REDALERT.INI` / `Run SETUP program first.` startup block is gone: startup now treats the config file as optional, only reads setup/private options when it exists, and otherwise continues into the normal window/audio/video/bootstrap path with defaults;
  - missing GameData no longer falls through into `Bootstrap()` with an invalid asset root: if none of the three allowed locations contains `REDALERT.MIX` plus `MAIN1.MIX`..`MAIN4.MIX`, startup now prints a console error describing the checked order and exits before window/bootstrap initialization;
  - validation for this checkpoint: incremental `cmake --build build --target redalert -- -j4`, `cmake --build build-asan --target redalert -- -j4`, `cmake --build build-emscripten --target redalert -- -j4`, and `ctest --test-dir build --output-on-failure` all succeed; a traced dummy-driver launch with `timeout -k 2s 12s env RA_TRACE_STARTUP=1 SDL_VIDEODRIVER=dummy SDL_AUDIODRIVER=dummy ./build/redalert -gamedata /home/lousy/git/EA/CnC_Red_Alert/GameData/` now selects `/home/lousy/git/EA/CnC_Red_Alert/GameData`, reaches `Main_Game(...)`, loads mix files from that directory, and stays alive until the external timeout window.
...
- Fixed the browser startup regression that surfaced after the GameData-only lookup cleanup:
  ...
  - practical browser failure: startup presence checks could still see synthetic manifest-backed top-level files such as `REDALERT.MIX` and the four `MAIN*.MIX` roots, so the page got far enough to create the window, but the later `CCFileClass("MAIN.MIX").Is_Available()` probe did **not** consult the manifest-backed synthetic view and therefore failed before `CONQUER.MIX` / `GENERAL.MIX` bootstrap validation, producing the in-game `Required game data was not found.` dialog even though the docker/web deployment contained the right files;
...
- Audit follow-up on the staged CD-removal / local-install branch fixed one remaining secondary-root edge case before the later exact-order cleanup removed that fallback entirely:
  - `CODE/SESSION.CPP` then skipped the loose `*.PKT` / `*.MPR` second-root glob when that optional install-root fallback was absent, which was the expected setup when the binary was launched directly from the selected asset directory;
  - before this guard, the multiplayer scenario enumeration path still called `WWFS_GlobDirectory(NULL, ...)`, which only returned no matches but also set an SDL error spuriously during menu-side loose-file discovery;
  - practical effect at that checkpoint: the staged optional second-root fallback still worked for installs with loose expansion packets/maps beside the executable, while direct asset-directory launches no longer injected a bogus filesystem error state into the same menu flow.
  - validation for this checkpoint: `cmake --build build --target redalert -- -j4`, `cmake --build build-asan --target redalert -- -j4`, `ctest --test-dir build --output-on-failure`, and the 20-second dummy-video ASan smoke (`timeout -k 5s 20s ... SDL_VIDEODRIVER=dummy ...`) succeed after the guard fix.

- Closed the last known score-screen asset-order gap in the staged CD-removal branch:
  - deeper comparison against the old `Force_CD_Available(...)` lifetime showed one remaining difference even after the earlier logical-view fixes: the port only refreshed the safe front-end logical-disc mixes when the requested `MAIN*` view changed, but a normal mission-complete flow keeps the same scenario view mounted from mission start through win movie and score presentation;
  - because the staged runtime shadows long-lived gameplay mixes (`CONQUER.MIX`, `SOUNDS.MIX`, `SPEECH.MIX`, `RUSSIAN.MIX`, `ALLIES.MIX`, expansion scenario `.MIX` files) with `Promote()`, those gameplay registrations could still sit ahead of the score/front-end carriers by the time `Score.Presentation()` asked for `TIMEHR.SHP`, `HISC1-HR.SHP`, `HISC2-HR.SHP`, and `BAR3*.SHP`;
  - `CODE/INIT.CPP` now splits logical-view updates into two phases: a view-change path that refreshes `MAIN.MIX`, re-shadows the long-lived gameplay families, and re-registers expansion scenario mixes, plus an unconditional front-end refresh that re-promotes/replaces the safe carriers (`MAIN.MIX`, `MOVIES*.MIX`, `GENERAL.MIX`, `SCORES.MIX`) on top of whatever gameplay shadowing is already active;
  - practical score-screen effect: `Configure_Score_Presentation_Main_Mix()` now refreshes the score/front-end namespace even when the scenario keeps the same logical `MAIN*` view, so the casualties ornaments and bar shapes resolve from the intended score assets instead of from gameplay-first mix ordering left behind by mission startup.
  - validation for this checkpoint: `cmake --build build --target redalert -- -j4`, `cmake --build build-asan --target redalert -- -j4`, and `ctest --test-dir build --output-on-failure` succeed, and a short dummy-video ASan smoke (`timeout -k 5s 20s ... SDL_VIDEODRIVER=dummy ...`) runs until the external timeout window (`124`) without `AddressSanitizer` or `runtime error:` output.

- Follow-up score-screen root-cause fix after the previous refresh-only attempt still left the casualties ornaments static and the bar art dark:
  - targeted runtime tracing of the actual score asset names (`BAR3BHR.SHP`, `BAR3RHR.SHP`, `TIMEHR.SHP`, `HISC1-HR.SHP`, `HISC2-HR.SHP`) showed that they resolve from `CONQUER.MIX`, not from `SCORES.MIX` or `GENERAL.MIX`;
  - the old working `Force_CD_Available(...)` path explicitly reloaded only `MAIN.MIX`, `MOVIES*.MIX`, `GENERAL.MIX`, and `SCORES.MIX` during disc changes, and it intentionally did **not** reload or replace `ConquerMix`;
  - the staged logical-view port had started shadowing `CONQUER.MIX` for runtime view changes, which is fine during gameplay, but score presentation was then still resolving those score-only SHPs from the most recently promoted gameplay `ConquerMix` rather than from the original bootstrap `ConquerMix` that the pre-removal runtime kept for this front-end flow;
  - `CODE/INIT.CPP` now remembers the bootstrap `ConquerMix` registration created during `Init_Secondary_Mixfiles()` and re-promotes that original mix only inside `Configure_Score_Presentation_Main_Mix()`, so gameplay/runtime view shadowing stays intact while the score screen reverts to the old base-archive score art source;
  - validation for this checkpoint: `cmake --build build --target redalert -- -j4`, `cmake --build build-asan --target redalert -- -j4`, and `ctest --test-dir build --output-on-failure` succeed, and the dummy-video ASan smoke (`timeout -k 5s 20s ... SDL_VIDEODRIVER=dummy ...`) again reaches the external timeout window (`124`) without `AddressSanitizer` or `runtime error:` output.

- Follow-up fix for the remaining score-screen regression after the movie-view repair:
  - deeper comparison against the pre-CD-removal `Force_CD_Available(...)` path showed that the old working runtime did not just change the logical disc alias; for the safe logical-disc families it *replaced* the active `MAIN.MIX`, `MOVIES*.MIX`, `GENERAL.MIX`, and `SCORES.MIX` registrations, which prevented later lookups from falling through into stale registrations from the previously mounted disc;
  - the staged logical-mix port had switched those families to a pure non-destructive `Promote()` model, which is necessary for long-lived gameplay/type-table mixes such as `CONQUER.MIX`, `RUSSIAN.MIX`, `ALLIES.MIX`, and scenario `.MIX` files, but it also let score-only assets such as `TIMEHR.SHP`, `HISC1-HR.SHP`, `HISC2-HR.SHP`, and `BAR3*.SHP` resolve from older logical-disc registrations when the newly selected score/general namespace did not provide the same file set;
  - `CODE/INIT.CPP` now keeps the non-destructive shadowing behavior for the risky long-lived gameplay/art mix families, but restores the old authoritative replacement behavior for `MAIN.MIX`, `MOVIES*.MIX`, `GENERAL.MIX`, and `SCORES.MIX` during logical-disc-view changes so the score screen cannot blend assets from multiple split-disc roots;
  - validation for this checkpoint: `cmake --build build --target redalert -- -j4`, `cmake --build build-asan --target redalert -- -j4`, and `ctest --test-dir build --output-on-failure` succeed after the score-screen mix-family replacement adjustment.

- Fixed the remaining logical-disc-view regressions from the staged CD-runtime removal without reintroducing CD globals or drive probing:
  - root-cause follow-up from the last known-good pre-removal state: the old `Force_CD_Available(...)` disc-swap path reloaded the currently visible `MOVIES1.MIX` / `MOVIES2.MIX` registration whenever the active disc changed, but the logical `MAIN.MIX` replacement in `CODE/INIT.CPP` only re-shadowed gameplay/audio/score mixes; that left Soviet single-player startup still on the old movie namespace even after rebinding `MAIN.MIX` to `MAIN2`, so `SCU01EA` could start with the right scenario data view while failing to expose the Soviet intro movie assets.
  - `CODE/INIT.CPP::Load_Movie_Mixfiles()` now always registers the movie mix for the current logical `MAIN.MIX` view, and `Configure_Main_Mix_View(...)` refreshes that movie registration immediately after rebinding the `MAIN.MIX` alias so side/expansion movie lookups follow the same logical-disc transition points that the old CD swap path used.
  - score-presentation follow-up: the original runtime did not perform a second disc swap between mission completion and the score screen; it kept the scenario-selected disc mounted through the win movie and score presentation. `Configure_Score_Presentation_Main_Mix()` therefore now prefers `Scen.ScenarioName` rather than collapsing to a house-based base view too early, with the old `HOUSE_BAD` Soviet-side fallback retained only for non-scenario edge cases.
  - validation for this checkpoint: `cmake --build build --target redalert -- -j4`, `cmake --build build-asan --target redalert -- -j4`, and `ctest --test-dir build --output-on-failure` succeed after the logical movie/score view fix.

- Fixed the cd-less expansion campaign regression without reintroducing CD runtime state:
  - root cause: the local install keeps split top-level disc roots (`MAIN1.MIX`..`MAIN4.MIX`), and the original code depended on a logical `MAIN.MIX` indirection over those disc variants; flattening or defaulting that layer to one fixed disc view left the Counterstrike campaign missions invisible because `Expansion_Dialog()` only populates entries when hardcoded campaign INIs such as `SCG20EA.INI` / `SCU31EA.INI` are visible through the currently registered mix chain;
  - practical archive finding: Counterstrike single-player campaign files resolve through the `MAIN3.MIX` view, while Aftermath campaign files resolve through the `MAIN4.MIX` view; the multiplayer `CSTRIKE.PKT` / `AFTMATH.PKT` packets are not the source for the hardcoded single-player `SCG*` / `SCU*` menu entries in `CODE/EXPAND.CPP`;
  - `SDL3_COMPAT/wrappers/sdl_fs.{h,cpp}` now provide a lightweight local `MAIN.MIX` alias plus a per-menu override so the SDL3 port can select the correct local split-disc root without reviving drive probing, disc prompts, or other CD-era runtime behavior;
  - `CODE/INIT.CPP` now keeps the logical `MAIN.MIX` registration, switches that alias before opening Counterstrike vs Aftermath mission dialogs, and registers the selected logical mix view (`MAIN.MIX`, `CONQUER.MIX`, `GENERAL.MIX`, `SCORES.MIX`, `RUSSIAN.MIX`, `ALLIES.MIX`, plus the expansion campaign scenario mix registrations) at the front of the mix lookup order so each expansion menu sees the correct campaign set without freeing older cached mix data that active type tables still reference;
  - follow-up fix: expansion missions also need the selected `SCORES.MIX` view shadowed and `ThemeClass::Scan()` rerun when the logical `MAIN.MIX` override changes, otherwise the mission can start under the right expansion data view but still use the startup score availability table from the wrong split-disc root, leaving in-mission music unavailable or incomplete;
  - broader runtime fix: the logical disc view cannot stay tied only to the expansion menu choice; the original game effectively used `MAIN1` for base Allied single-player content, `MAIN2` for base Soviet single-player content, `MAIN3` for Counterstrike content (including ants / Counterstrike multiplayer naming), and `MAIN4` for Aftermath content, so `CODE/INIT.CPP`, `CODE/SCENARIO.CPP`, and `CODE/SAVELOAD.CPP` now rebind the logical mix view from the actual scenario name on scenario start/save-load and restore a base/common view for front-end and score-presentation paths instead of letting expansion state leak across base/ex1/ex2 transitions;
  - companion runtime follow-up: those rebinds now shadow `SOUNDS.MIX` and `SPEECH.MIX` alongside `CONQUER.MIX`, `GENERAL.MIX`, `SCORES.MIX`, `RUSSIAN.MIX`, and `ALLIES.MIX`, and the SDL filesystem `MAIN.MIX` fallback order now prefers `MAIN1` before the other split-disc roots so startup/common caches come from the base game instead of starting in an Aftermath-biased view;
  - `CODE/MIXFILE.{H,CPP}` now expose `MixFileClass::Promote(...)` so a newly registered logical mix can shadow older registrations with the same basename instead of destroying them; this avoids the expansion-start use-after-free caused by long-lived art pointers returned from `MFCD::Retrieve(...)`;
  - validation for this checkpoint: `cmake --build build --target redalert -- -j4`, `cmake --build build-asan --target redalert -- -j4`, and `ctest --test-dir build --output-on-failure` succeed after the broader mix-view rebind fix; a long dummy-video ASan smoke (`timeout -k 5s 185s ... SDL_VIDEODRIVER=dummy ...`) holds for the full timeout window (`124`), while a traced normal-window smoke in this shell still exits cleanly via `Main window close request` / `Main window destroy` after about 18 seconds rather than via ASan/UBSan.

- Removed the remaining CD-era asset/runtime layer so the SDL3 port now assumes a fixed local `GameData/` install and keeps DVD-specific behavior limited to movies behind a compile-time gate:
  - deleted the old removable-media support files (`CODE/CDFILE.{H,CPP}` and the `WIN32LIB/PLAYCD` helpers), removed the matching public declarations/globals (`RequiredCD`, `CurrentCD`, `Get_CD_Index`, `Force_CD_Available`, `Get_CD_Drive`), and rewired `CODE/CCFILE.{H,CPP}` to inherit directly from `BufferIOFileClass` instead of preserving the old CD search-drive abstraction under a different name;
  - simplified startup/media initialization in `CODE/INIT.CPP` to validate the required local archives in `GameData/` instead of probing discs or honoring `-CD...` search-path overrides, and removed the remaining high-level disc-swap handling from `CODE/{SCENARIO,SAVELOAD,NETDLG,WOL_GSUP,STARTUP,MENUS,GADGET}.CPP` so scenario, save/load, multiplayer, and WOL flows now rely on ordinary local file availability;
  - removed the WWFS virtual-CD path emulation from `SDL3_COMPAT/wrappers/sdl_fs.{h,cpp}` and the dependent fake-CD reporting from `SDL3_COMPAT/wrappers/win32_compat.cpp`, so the filesystem layer no longer pretends drive letters map to mounted Red Alert discs on case-sensitive SDL platforms;
  - added the `RA_ENABLE_DVD_MOVIES` CMake option and confined the remaining DVD-specific behavior to movie handling only: `CODE/INTRO.{H,CPP}` now uses an explicit allied/soviet side choice for the optional DVD movie flow, and `CODE/INIT.CPP` only loads the extra movie mix when that compile-time option is enabled and the local movie archives are present;
  - follow-up fix for the first pass: startup asset lookup now uses one strict directory order and stops at the first hit: `-gamedata <dir>`, then the process current working directory, then the executable directory. `CODE/STARTUP.CPP` no longer probes sibling/parent `GameData/` paths, and `CODE/{CCFILE,INIT,SESSION}.CPP` no longer keep a second launch-directory fallback for loose `SC*.MIX` / `SS*.MIX` / `*.PKT` / `*.MPR` content;
  - the same pass keeps the existing split-disc bootstrap requirement intact: `CODE/INIT.CPP` still registers the fixed top-level `MAIN1.MIX`..`MAIN4.MIX` archives before validating logical inner mixes like `CONQUER.MIX`, `GENERAL.MIX`, `SOUNDS.MIX`, and the movie archives, while the Emscripten lazy-fetch build still resolves the executable-directory step to the persistent cache root rather than reintroducing extra search heuristics;
  - validation for this checkpoint: `cmake --build build --target redalert -- -j4`, `cmake --build build-asan --target redalert -- -j4`, `cmake --build build-emscripten --target redalert -- -j4`, and `ctest --test-dir build --output-on-failure` succeed after the search-order replacement.
...
- Followed the dead-guard purge with a call-tree cleanup so the tree no longer keeps local no-op wrappers that only survived because their guarded bodies were deleted:
  ...
  - removed the now-empty `Enable_Secret_Units()` helper from `CODE/CONQUER.CPP`, deleted its forward/extern declarations, and dropped the special `"SECRET UNITS ON "` / `"XECRET UNITS ON "` message rewriting that only existed to trigger that helper during multiplayer chat processing;
  - validation for this checkpoint: `cmake --build build --target redalert -j4`, `ctest --test-dir build --output-on-failure`, and `cmake --build build-asan --target redalert -j4` all succeed after the follow-up cleanup; the timed ASan/UBSan startup smoke run `cd GameData && timeout -k 5s 20s env ASAN_OPTIONS=abort_on_error=1:detect_leaks=0 UBSAN_OPTIONS=print_stacktrace=1 ../build-asan/redalert` still exits only via the external timeout window (`137`), and the captured log contains no `AddressSanitizer` or `runtime error:` matches.

- Removed the dead preprocessor branches guarded by `OBSOLETE` and the matching legacy guard family from the active source tree without changing compiled behavior:
  - audited every `#ifdef OBSOLETE` section plus the similar permanently-disabled guards `NEVER`, `OLDWAY`, `OLD`, and the handful of adjacent `#if 0` blocks in project code, then collapsed them to the code path that already builds today because those guards are never defined in the SDL3/CMake build;
  - deleted about 3.1k lines of dead alternative implementations across 53 files in `CODE/` and `WIN32LIB/`, including the old multiplayer side-selection UI in `CODE/NETDLG.CPP`, never-compiled DOS/debug/helper code in files such as `CODE/{CDFILE,CCFILE,LZW,MPLAYER,VERSION}.CPP`, and dormant gameplay branches such as the old nuke/thief/AI snippets in `CODE/{BUILDING,INFANTRY,HOUSE,UNIT}.CPP`;
  - representative high-risk confirmations from the sweep: `CODE/CELL.CPP::Concrete_Calc()` was already compiling as an empty no-op because its entire body lived under `#ifdef OBSOLETE` and was then removed entirely in the follow-up call-tree cleanup, `CODE/LZO_CONF.H` kept the existing safe `LZO_CHECK_MPOS_DET(...)` macro by dropping the disabled `#if 0` variant, and `CODE/NETDLG.CPP` now keeps only the active house-dropdown path instead of carrying the commented `OLDWAY` selector path alongside it;
  - validation for this checkpoint: `cmake --build build --target redalert -j4`, `ctest --test-dir build --output-on-failure`, and `cmake --build build-asan --target redalert -j4` all succeed after the cleanup; the timed ASan/UBSan startup smoke run `cd GameData && timeout -k 5s 20s env ASAN_OPTIONS=abort_on_error=1:detect_leaks=0 UBSAN_OPTIONS=print_stacktrace=1 ../build-asan/redalert` still exits only via the external timeout window (`137`), and the captured log contains no `AddressSanitizer` or `runtime error:` matches.
...
- Audited project-side `SDL_Delay` / sleep usage and removed the only live manual display-sync wait that was duplicating SDL renderer vsync:
  ...
  - the later pacing follow-up removed the active callback-driven `SDL_Delay(1)` waits from `CODE/{CONQUER,EVENT,LOADDLG,SCENARIO,SCORE}.CPP` and from `CODE/PALETTEC.CPP` by routing those paths through normal callback/present work instead of one-millisecond sleeps; the remaining non-vendor `SDL_Delay(1)` / `SDL_GameInput_Delay(1)` sites are now concentrated in input debounce, UDP/WOL polling, and a few vendor/library compatibility waits rather than gameplay or front-end presentation pacing;
  ...
  - validation for this checkpoint: incremental `cmake --build build --target redalert -j4`, `ctest --test-dir build --output-on-failure`, incremental `cmake --build build-asan --target redalert -j4`, and the timed ASan/UBSan startup smoke run `cd GameData && timeout -k 5s 20s env ASAN_OPTIONS=abort_on_error=1:detect_leaks=0 UBSAN_OPTIONS=print_stacktrace=1 ../build-asan/redalert` all succeed after the delay/vblank audit change; the timed smoke run still exits only via the external timeout/SIGKILL window (`137`), and the captured log contains no `AddressSanitizer` or `runtime error:` matches.

- Reworked gameplay frame scheduling and callback waits so the SDL present path owns display cadence instead of old in-loop sleeps:
  - deleted `CODE/CONQUER.CPP::Sync_Delay()` and replaced the old "run one logic frame, then wait in place" model with a frame-ready check (`Main_Loop_Frame_Interval()` / `Main_Loop_Frame_Ready()` / `Run_Game_Frame()`) so each `Main_Loop()` call still runs at most one gameplay frame while outer-loop callback/render work continues every iteration;
  - follow-up cleanup after in-game slowdowns: removed the temporary multi-step accumulator/catch-up logic and restored the original one-frame-per-call behavior, because batching several gameplay steps into one `Main_Loop()` call starved rendering and changed the way the old engine shed load;
  - added `Call_Back_Wait(1)` plus SDL draw helpers `WWDraw_Has_Pending_Present()` / `WWDraw_Request_Present()` so short speech/countdown/score/palette waits can keep using the callback path and fall back to presenting the current primary surface instead of calling `SDL_Delay(1)`;
  - immediate follow-up fix: `Call_Back_Wait(1)` must stay callback-only and must not run `Color_Cycle()` internally; several non-gameplay waits (score/load/scenario speech paths) rely on their currently active palette staying untouched, and routing them through the in-game color-cycle path caused palette corruption/regressions;
  - rewired `CODE/{EVENT,LOADDLG,SCENARIO,SCORE}.CPP` and `CODE/PALETTEC.CPP` to use that callback/present path, simplified `CODE/SCORE.CPP::Call_Back_Delay(...)` so score animations now stay on the normal callback path rather than mixing a private audio-only timer with `SDL_GameInput_Delay(1)`, and restored map rendering during playback now that the old `Sync_Delay()` loop no longer exists to redraw while waiting;
  - intentionally left the short `SDL_Delay(1)` sites in `CODE/NETDLG.CPP` and the low-level keyboard/input shims alone in this pass because they are still serving non-render polling/yield purposes rather than display synchronization;
  - validation for this checkpoint: `cmake --build build -- -j2` and `ctest --test-dir build --output-on-failure` both succeed after the gameplay frame scheduling refactor.
...
- Removed the last runtime opt-out from SDL audio so the port now only runs with the SDL backend active:
  ...
  - finished the backend-side cleanup in `WIN32LIB/AUDIO/SOUNDIO.CPP` and `WIN32LIB/{AUDIO,INCLUDE}/AUDIO.H` by removing the leftover SDL-vs-none state globals/prototypes, and by reporting SDL audio init failures as hard errors rather than optional warnings;
  - follow-up audit for project-side SDL switches only found vendored SDL build toggles and the unused localization token comments for `TXT_NO_SOUND_CARD`; no additional project runtime/backend selection path remains outside the supported SDL implementation;
  - validation for this checkpoint: incremental `cmake -S . -B build && cmake --build build --target redalert -- -j2`, incremental `cmake -S . -B build-asan && cmake --build build-asan --target redalert -- -j2`, `ctest --test-dir build --output-on-failure`, clean `cmake -S . -B build && cmake --build build --target clean -- -j2 && cmake --build build --target redalert -- -j2`, and clean `cmake -S . -B build-asan && cmake --build build-asan --target clean -- -j2 && cmake --build build-asan --target redalert -- -j2` all succeed after the SDL-only audio cleanup.

- Removed the last active source-level multi-option backend seams so the SDL port now exposes one supported path for drawing and audio instead of carrying old fallback/policy choices:
  - deleted the dead Win32 profiler glue from the supported tree by removing `SDL3_COMPAT/wrappers/profile_compat.cpp`, the unused `WIN32LIB/INCLUDE/PROFILE.H` header, the shutdown-only `Stop_Profiler()` call in `CODE/WINSTUB.CPP`, and the stale include links from `WIN32LIB/{INCLUDE,MISC}/WWLIB32.H` plus `WIN32LIB/TIMER/TIMERINI.CPP`;
  - collapsed the remaining DirectDraw-era hidden-page selection logic in `CODE/STARTUP.CPP` so the SDL path always creates the hidden page as a video surface, removing the legacy `VideoBackBuffer` / `HardwareFills` setup options, the raw-buffer fallback branch, and the old fake-capability queries from `WIN32LIB/MISC/DDRAW.CPP` / `WIN32LIB/{MISC,INCLUDE}/MISC.H`;
  - followed that drawing cleanup through the SDL adapter by dropping the unused system-memory surface parameter and dead capability-reporting methods from `SDL3_COMPAT/wrappers/sdl_draw.{h,cpp}`, and by updating the remaining callers in `WIN32LIB/DRAWBUFF/{GBUFFER,ICONCACH}.CPP` plus the duplicated include headers so only the supported SDL video-surface path remains in active code;
  - simplified the sound backend declarations in `WIN32LIB/{AUDIO,INCLUDE}/AUDIO.H` and `WIN32LIB/AUDIO/SOUNDIO.CPP` by removing the unused `Score_Type` driver enum baggage and the stale pre-SDL `Sound_Init(...)` / `Audio_Init(sample,address,interrupt,dma)` prototypes from both `WIN32LIB/*/FUNCTION.H` copies; the later SDL-only audio pass then removed the temporary `SoundType` / `SampleType` stopgaps entirely;
  - validation for this checkpoint: incremental `cmake -S . -B build && cmake --build build --target redalert -- -j2`, incremental `cmake -S . -B build-asan && cmake --build build-asan --target redalert -- -j2`, clean `cmake -S . -B build && cmake --build build --target clean -- -j2 && cmake --build build --target redalert -- -j2`, `ctest --test-dir build --output-on-failure`, and clean `cmake -S . -B build-asan && cmake --build build-asan --target clean -- -j2 && cmake --build build-asan --target redalert -- -j2` all succeed after the cleanup.

- Removed the remaining obsolete DOS/Win32-era assembly and direct-video scaffolding from the active tree without changing the SDL runtime behavior:
  - deleted the repo-only assembler sources/includes under `WIN32LIB/` (`*.ASM`, `*.INC`, `*.I`) plus the unused legacy video/port-I/O headers (`WIN32LIB/INCLUDE/VIDEO.H`, `WIN32LIB/INCLUDE/VQM32/{PORTIO,VIDEO,VESAVID,REALMODE}.H`) so the source tree no longer advertises unsupported VGA/VESA/real-mode implementations that the CMake build never compiled;
  - removed `CODE/PORTIOCOMPAT.CPP` and rewired `CODE/RGB.{H,CPP}` to call `Set_Palette_Register(...)` directly instead of emulating VGA DAC register writes through fake `outportb(...)` / `outrgb(...)` helpers;
  - scrubbed the permanently-disabled VQA direct-video branches from `WIN32LIB/VQA32/{CONFIG,DRAWER,LOADER,UNVQ,VQAPLAYP}.CPP/.H`, leaving only the buffered SDL movie path in the active source and replacing the deleted `VQM32/VIDEO.H` dependency with local declarations for the still-used VBI helper seam;
  - companion cleanup from the same sweep removed stale build exclusions for already-deleted legacy sources from `CMakeLists.txt`, deleted the unused `ModeXBuff` global plus the one-line `ModeX_Blit(...)` wrapper, and rewrote lingering DirectDraw / MCGA / VESA comments plus dead VQA mode flags so the live tree better matches the current SDL-backed implementation;
  - validation for this checkpoint: clean `cmake -S . -B build && cmake --build build --target clean -- -j2 && cmake --build build --target redalert -- -j2`, clean `cmake -S . -B build-asan && cmake --build build-asan --target clean -- -j2 && cmake --build build-asan --target redalert -- -j2`, and `ctest --test-dir build --output-on-failure` all succeed after the cleanup.
...
- Fixed a score-screen streamed-music restart regression in the SDL port without changing the original intended end-of-track loop behavior:
  ...
  - the original SDL regression was that `CODE/SCORE.CPP::Call_Back_Delay(...)` could let streamed music go idle during long waits; the later pacing refactor removed that special throttled callback path entirely, and score delays now stay on the normal `Call_Back()` / present cadence through `Call_Back_Wait(1)` instead of a private `Sound_Callback()` timer plus `SDL_GameInput_Delay(1)`;
...
- Switched the SDL/Linux mouse confinement over to gameplay-only mission capture so tactical border scrolling can keep the pointer locked during live play without trapping it on menus, movies, or other front-end UI:
  ...
  - validation for this checkpoint: `cmake --build build -j4`, `ctest --test-dir build --output-on-failure`, `cmake --build build-asan --target redalert -j4`, and `cd GameData && timeout -k 5s 20s env ASAN_OPTIONS=abort_on_error=1:detect_leaks=0 UBSAN_OPTIONS=print_stacktrace=1 ../build-asan/redalert` all succeed after the mouse-capture change; the timed ASan/UBSan smoke run still exits only via the external timeout/SIGKILL window (`137`), and the captured log contains no `AddressSanitizer` or `runtime error:` matches.
...
- Fixed the SDL palette fading regressions while preserving the original game behavior:
  ...
  - validation for this checkpoint: `cmake --build build --target redalert -j8`, `cmake --build build-asan --target redalert -j8`, and `cd GameData && timeout -k 5s 20s env ASAN_OPTIONS=abort_on_error=1:detect_leaks=0 UBSAN_OPTIONS=print_stacktrace=1 ../build-asan/redalert` all complete as expected after the fade fixes; the timed ASan/UBSan smoke run still exits only via the external timeout/SIGKILL window (`137`), and the captured log contains no `AddressSanitizer` or `runtime error:` matches.
...
- Fixed a mission-table out-of-bounds gameplay crash in the SDL/Linux ASan build without changing intended targeting or team-selection behavior:
  ...
  - validation for this checkpoint: `cmake --build build --target redalert -j8`, `cmake --build build-asan --target redalert -j8`, and `cd GameData && timeout -k 5s 20s env ASAN_OPTIONS=abort_on_error=1:detect_leaks=0 UBSAN_OPTIONS=print_stacktrace=1 ../build-asan/redalert` all succeed after the fix; the timed ASan/UBSan smoke run still exits only via the external timeout/SIGKILL window (`137`), and the captured log contains no `AddressSanitizer` or `runtime error:` matches.
...
- Fixed the reported SDL/Linux ASan/UBSan gameplay startup failures without changing intended behavior:
  ...
  - validation for this checkpoint: `cmake --build build --target redalert -j8`, `cmake --build build-asan --target redalert -j8`, and `cd GameData && timeout -k 5s 20s env ASAN_OPTIONS=abort_on_error=1:detect_leaks=0 UBSAN_OPTIONS=print_stacktrace=1 ../build-asan/redalert` all succeed as expected after the fix; the timed ASan/UBSan smoke run still exits only via the external timeout/SIGKILL window (`137`), and the captured log contains no `AddressSanitizer` or `runtime error:` matches.
...
- Fixed a radar/minimap scroll redraw regression in the SDL port without changing gameplay behavior:
  ...
  - validation for this checkpoint: `cmake --build build --target redalert -j8`, `cmake --build build-asan --target redalert -j8`, and `cd GameData && timeout -k 5s 20s env ASAN_OPTIONS=abort_on_error=1:detect_leaks=0 UBSAN_OPTIONS=print_stacktrace=1 ../build-asan/redalert` all complete as expected after the fix; the ASan smoke run still exits only via the external timeout/SIGKILL window (`137`) with no `AddressSanitizer` or `runtime error:` matches in the captured log.
...
- Completed the first Emscripten bring-up with SDL main callbacks and a linked web target:
  ...
  - validation for this checkpoint: `cmake --build build-emscripten --target redalert -j8` now succeeds, `cmake --build build --target redalert -j8` still succeeds, `cmake --build build-asan -j8` succeeds, and `cd GameData && timeout 20s env ASAN_OPTIONS=abort_on_error=1:detect_leaks=0 UBSAN_OPTIONS=print_stacktrace=1 ../build-asan/redalert` completes without `AddressSanitizer` or `runtime error:` matches in the captured log; the current local `build-emscripten/` cache was configured with `RA_EMSCRIPTEN_PACKAGE_GAMEDATA=ON`, so that successful web build also produced `build-emscripten/redalert.data` with Emscripten's expected warning about a roughly `1.6 GB` asset bundle;
...
- Fixed an ASan-reported edge-follow path buffer overflow in `CODE/FINDPATH.CPP` without changing pathfinding behavior:
  ...
  - validation for this checkpoint: `cmake --build build --target redalert -j8`, `cmake --build build-asan --target redalert -j8`, and `cd GameData && timeout 20s env ASAN_OPTIONS=abort_on_error=1:detect_leaks=0 UBSAN_OPTIONS=print_stacktrace=1 ./redalert-asan` all succeed after the fix, with the runtime probe exiting only via `timeout` and no `AddressSanitizer` / `runtime error:` matches in the captured log.
...
- Fresh validation for that follow-up now passes on the current tree:
  ...
  - a focused `timeout 30s env ASAN_OPTIONS=abort_on_error=1:detect_leaks=0:new_delete_type_mismatch=0 ./redalert-asan` smoke run from `GameData/` reaches the debug-map mission path and times out with only the already-known `CODE/SHA.CPP` misaligned-load and `CODE/RANDOM.CPP` left-shift warnings;

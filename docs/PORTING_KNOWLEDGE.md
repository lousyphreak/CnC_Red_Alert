...
- The fixed local install still needs the active install directory itself registered as the one local search root after startup validation.
  - Practical example from this tree: `CODE/STARTUP.CPP` now switches the process cwd into the resolved `GameData/` directory so the fixed top-level archives are found consistently, but some installs still keep loose official `SC*.MIX` / `SS*.MIX` / `*.PKT` / `*.MPR` files beside the executable or repo root instead of inside `GameData/`. `CODE/CCFILE.{H,CPP}` therefore keeps one secondary local fallback root pointing at that original launch directory, and `CODE/{INIT,SESSION}.CPP` explicitly scan both locations for loose scenario content.
  - Local repo/layout note: the checked-in source tree used for this port does not itself contain loose `SC*.MIX`, `SS*.MIX`, `*.PKT`, or `*.MPR` files under the repo root or `GameData/`; this fallback exists for real local installs/user layouts that keep those official expansion files beside the executable instead of inside `GameData/`.
  - Practical rule: preserving one secondary local install-root fallback is not the same thing as preserving CD-era media probing. Keep the resolved `GameData/` cwd for the fixed archive set, and if startup needs a second path at all, keep it limited to the original local launch directory rather than reviving `-CD...` overrides, drive-letter probing, or disc prompts.
  - Implementation edge-case rule: treat that secondary local root as optional. If startup resolved no separate launch-directory fallback, callers must skip loose-file globbing instead of passing a null path into `WWFS_GlobDirectory(...)`; otherwise the scan just returns no matches but also leaves a spurious SDL filesystem error behind.

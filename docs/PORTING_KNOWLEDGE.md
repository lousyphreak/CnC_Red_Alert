...
- Browser data packaging must stay explicit.
  ...
  - That distinction matters for startup CD probing: `Get_CD_Index(...)` and the virtual-drive helpers probe for `MAIN*.MIX` presence during disc detection. If those checks fetched the real archives, startup would immediately redownload almost the whole game; manifest-backed presence checks avoid that while still letting the later real `MAIN.MIX` open fetch the selected disc archive on demand.
  ...
  - Practical consequence: the initial page download is now the HTML/JS/wasm plus the tiny embedded manifest, while first-run startup only downloads the specific archives it actually opens (for example `REDALERT.MIX` and whichever `MAIN*.MIX` the virtual CD logic selects). An actual in-browser smoke test is still needed to pin down the exact first-run fetch set and cache behavior across reloads.

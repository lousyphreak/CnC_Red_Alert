...
- Browser data packaging must stay explicit.
  ...
  - There is now a second important split inside the "open and read" path:
    ...
    - read-only remote `.MIX` opens now go through a custom `SDL_OpenIO(...)` stream in `SDL3_COMPAT/wrappers/sdl_fs.cpp`, which probes the archive size with `Range: bytes=0-0`, keeps a per-archive chunk cache, and fetches 256 KiB ranges on demand as the old game code seeks/reads through the archive;
    ...
    - the range stream batches consecutive missing chunks into one HTTP request (currently capped at 8 chunks / 2 MiB per fetch), which matters because original flows like `MixFileClass<T>::Cache(...)` and large sequential `RawFileClass::Read(...)` calls would otherwise degenerate into a long chain of 256 KiB network round-trips even when the game was clearly consuming a contiguous span;
    - this works well with the original archive code because `MixFileClass<T>::MixFileClass(...)` only needs the archive header/index first, later `CCFileClass`/`RawFileClass::Bias(...)` reads naturally collapse to exact subfile byte ranges inside the parent archive, and full-archive cache requests still only fetch the bytes the game actually streams through.

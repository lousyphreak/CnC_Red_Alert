...
- Browser data packaging must stay explicit.
  ...
  - There is now a second important split inside the "open and read" path:
    ...
    - read-only remote `.MIX` opens now go through a custom `SDL_OpenIO(...)` stream in `SDL3_COMPAT/wrappers/sdl_fs.cpp`, which probes the archive size with `Range: bytes=0-0`, shares a per-archive in-memory chunk cache, and fetches 256 KiB ranges on demand as the old game code seeks/reads through the archive;
    - this works well with the original archive code because `MixFileClass<T>::MixFileClass(...)` only needs the archive header/index first, and later `CCFileClass`/`RawFileClass::Bias(...)` reads naturally collapse to exact subfile byte ranges inside the parent archive.
  ...
  - The current `.MIX` range cache is session-memory-only.
    - Practical consequence: repeat opens of the same archive during one page session reuse already downloaded chunks instead of re-requesting them, which is the important fix for startup/bootstrapping behavior.
    - Current limitation: sparse chunks are not yet persisted across full page reloads; only the whole-file lazy-fetch path writes to IDBFS today.
  ...
  - Practical consequence: the initial page download is now the HTML/JS/wasm plus the tiny embedded manifest, while first-run startup only downloads the specific archives it actually opens (for example `REDALERT.MIX` and whichever `MAIN*.MIX` the virtual CD logic selects), and large `.MIX` archives no longer need to download whole-file payloads before those embedded reads can begin. An actual in-browser smoke test is still needed to pin down the exact first-run range pattern and to decide whether sparse chunk persistence across reloads is worthwhile.
